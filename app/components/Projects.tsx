import { HiExternalLink, HiFolderOpen } from 'react-icons/hi';

export default function Projects() {
  const projects = [
    {
      title: 'Portfolio and Blogging website',
      desc: 'A full stack system to share your introduction and blogs in one website.',
      tech: 'Next.js, Tailwind, Node.js, MongoDB, Vercel',
      demo: 'https://sheikh-ubaid-web.vercel.app/',
    },
    {
      title: 'Portfolio Website',
      desc: 'Professional developer portfolio built with Next.js and Tailwind.',
      tech: 'Next.js, Tailwind',
      demo: 'https://sheikh-ubaid-web.vercel.app/',
    },
  ];

  return (
    <section id="projects" className="max-w-6xl px-4 py-20 mx-auto">
      <h2 className="mb-12 text-3xl font-bold text-center inline-flex items-center gap-3">
        <HiFolderOpen className="w-6 h-6 text-blue-600" />
        Projects
      </h2>
      <div className="grid gap-10 md:grid-cols-2">
        {projects.map((project) => (
          <div key={project.title} className="p-6 transition border rounded-lg bg-white hover:shadow-lg">
            <h3 className="mb-2 text-xl font-semibold flex items-center gap-2">
              <HiFolderOpen className="w-5 h-5 text-blue-600" />
              {project.title}
            </h3>
            <p className="mb-2 text-gray-700">{project.desc}</p>
            <p className="mb-4 font-mono text-sm text-gray-500">{project.tech}</p>
            <div className="flex items-center gap-2">
              <a href={project.demo} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-700">
                Live Demo
                <HiExternalLink className="w-4 h-4" />
              </a>
            </div>
          </div>
        ))}
      </div>
    </section>
  )
}
